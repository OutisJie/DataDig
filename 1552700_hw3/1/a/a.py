...
def reverse(pred, lon1, lat1, grid_x_num, grid_lon_len, grid_lat_len):
    ...
    result['Longitude'] = (result['grid_x'] + 0.5) * grid_lon_len + lon1
    result['Latitude'] = (result['grid_y'] + 0.5) * grid_lat_len + lat1
...
def initDataSet(data):
    ...
    return training_set, test_set, training_labels, test_labels, test_pos
...
if __name__ == '__main__':
    ...
    for i in range(0, 10):
        # 整理训练集和测试集
        training_set, test_set, training_labels, test_labels, test_pos = initDataSet(data_set)
        ...
        for index in range(0, 6):
            ...
            times[index].append(round(end - start, 2))
            # 将得到的gridID转换为经纬度
