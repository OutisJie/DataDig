...
def find_similar(data_set, K, topk_minus_index):
    base_location = []
    similar_group = []
    for group in data_set:
        base_location.append([group['Longitude_1'][0], group['Latitude_1'][0]])
    base_location = np.array(base_location)
    # 此时得到最大聚类为9
    cluster = KMeans(n_clusters=12, random_state=10)
    cluster_labels = cluster.fit_predict(base_location)
    print(cluster_labels)
    for index in topk_minus_index:
        current_label = cluster_labels[index]
        group = []
        # 找到所有和index这个组一个聚类的所有组
        for pos in range(0, len(cluster_labels)):
            if cluster_labels[pos] == current_label and pos != index:
                group.append(data_set[pos])
        similar_group.append(group)
    # dic = {}
    # tops = []
    # max = 1
    # max_number = 0
    # for i in range(0, base_location.shape[0]):
    ...
    #     similar_group.append(data_set[tops[index]])
    print(len(similar_group))
    for i in range(0, len(similar_group)):
        print(len(similar_group[i]))
...
if __name__ == '__main__':
    ...
    for i in range(0, len(median_score)):
        if median_score[i] in topk_median_score_minus:
            topk_minus.append(data_set[i])
            topk_minus_index.append(i)
    
    similar_group = find_similar(data_set, K, topk_minus_index)
    ...
    for k in range(0, K):
        ...
        # 画图
        plt.plot(persents, old_average_score, 'ro-', label='old_data')

        # 对于加入了plus数据的新group
        new_group = topk_minus[k]
        temp = similar_group[k]
        for topk in temp:
            new_group = pd.concat([new_group, topk])
        #new_group = pd.concat([topk_minus[k], similar_group[k]])
        new_score_set= []
        ...
        # 画图
        plt.plot(persents, new_average_score, 'b*-', label='new_data')
